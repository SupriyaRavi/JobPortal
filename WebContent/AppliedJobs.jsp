...
<%@page import="JobPortal.Bean.TotalAppliedCandidates"%>
...
<input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search Based On Skills..">
	<table border="1" width="500" align="center" id="myTable">
		...
		<%
			List<TotalAppliedCandidates> J = (List<TotalAppliedCandidates>) request.getAttribute("TotalJobs");
		if(J!=null){
			for (TotalAppliedCandidates j : J) {
		%>
		<tr>
			...
			<td><form action="UpdateJobApplications" method="post"><a class="two" href="UpdateJobApplications?id=<%=j.getJobTitle()%>&comp=<%=j.getCompanyName()%>">YES</a></form></td>
		</tr>
		<%}}%>
	</table>
